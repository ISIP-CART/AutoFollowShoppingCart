...
为降低首次起步偏航，正式固件保留起步助推。直行/后退使用四轮同步助推；
左右同向但速度不同的缓弯命令使用按比例助推，避免助推把差速抹成直行；原地左转/
右转继续使用单独的四轮对称补偿，避免 `c-5,5` / `c5,-5` 悬空正常但落地顶不动。

- 仅在四轮 current 均为 0、收到新的非零 `c` 命令时触发；
- 触发前还要求最近 `$MSPD` 遥测新鲜，且四轮实测速度绝对值均不超过
  `STARTUP_ASSIST_MSPD_ZERO_LIMIT = 80.0`；
- 精确直行/后退持续 `STRAIGHT_STARTUP_ASSIST_MS = 1600 ms`：前
  `STRAIGHT_BREAKAWAY_KICK_MS = 350 ms` 使用四轮对称 `24` 破静摩擦，随后四轮最低
  输出均为 `20`；这条路径保持现有落地直行修复不变。
- 前进缓弯（例如 `c10,14` 或 `c14,10`）同样持续 `1600 ms`，但按照左右目标比例
  施加助推：`c10,14` 在 kick 阶段约为 `17:24`，保持阶段约为 `14:20`，而不会变成
  两侧相同输出。若车辆先以 `c14,14` 起步、助推窗口内才收到缓弯命令，固件会保留
  原助推计时并切换为比例化输出。协议仍然只有 `c<left>,<right>`。
...
- USB 诊断中 `drive_output` 会显示
  `startup_assist=1,startup_assist_mode=straight|curve|turn`，
  `!Q` 也会显示当前助推状态；
...
2. 车轮悬空测试：先由 USB 发送 `!D,1`，再由 Android BLE 依次验证
   `c14,14`、`c10,14`、`c14,10`、`c0,0`、`c-5,5`、`c5,-5`、`c-12,-12`；确认
   `c14,14` 仍为同步直行，两个缓弯命令的 `$spd` 左右比例相反且不触发换向阻止。每条
   BLE 命令都应有对应的 `motion_rx`、target/current 与 `$spd` 演变记录。
