...
- 已同步采购记录到 [design/采购.md](design/采购.md)。
- 已采购核心硬件包括：305 系列四驱底盘、ESP32 相关开发板套件、电池、充电器、AT8236 驱动板。
- 已采购装配辅材包括：杜邦线、手机支架、联轴器、铜柱、螺丝。
- 截至 2026-07-09，采购记录中的已购物料总额约为 830.65 元。
- 急停按钮、降压模块、OTG 线等物料按最新采购记录与实物情况核对；超声波模块的后续到货与测试进展见下节。

## 近场传感器测试进度（2026-09-03）

已购 URM09-I²C 超声波 ×1、VL53L1X 红外 ToF ×2，并取得超声波单独采集及双 ToF 连续采集的实际串口日志。现有 ESP32 拓展板 I²C 接线沿用 SDA=GPIO21、SCL=GPIO22，用户确认电源跳帽选择 3.3 V。

- [接线核验与测试记录总览](design/tests/near-field/README.md)
- [双 VL53L1X 测试报告](design/tests/near-field/VL53L1X测试报告.md)：左右各 1657 条，有效率 99.88% / 96.14%；右侧末段连续无效待复测。
- [URM09 测试报告](design/tests/near-field/URM09测试报告.md)：411 条，有效率 87.10%；存在连续无效值及局部距离跳变。

上述有效率按各自台架程序定义统计，不能直接作性能比较。三设备同时采集、固定距离精度、电机干扰及传感器触发整车停车尚未验收；报告已列出缺失证据和复测步骤，并附原始日志及可复算脚本。
