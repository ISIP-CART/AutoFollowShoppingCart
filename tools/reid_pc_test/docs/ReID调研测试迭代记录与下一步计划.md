...
## 20. 诊断分析后的下一步：先修正 Android ReID crop 方向（2026-07-08）

### 20.1 最新诊断结论

PC 侧新增 `analyze_cartfollow_diagnostics_v1.py` 后，对 4 组 Human Cart Simulator 诊断 session 做了首轮离线分析：

```text
target_return 事件数：11
belief_high_bbox_failed：7
hard_stop_before_return：2
candidate_switch_penalty：2
```

其中 `diagnostic_gallery_quality.csv` 显示旧版诊断 gallery 全部是 `landscape_or_rotated`。结合 Android 代码检查，可以确认旧版 ReID gallery 与 candidate crop 都可能没有按 `sensorOrientation` 转正就进入 TFLite ReID 模型。

因此下一步优先级不是继续调 `bestScore / margin / belief` 阈值，而是先保证 Android ReID 的输入 crop 方向正确、诊断 gallery 可人工复核。

### 20.2 已确定的 Android 修正方向

本轮 Android 修改目标：

```text
ReID gallery crop：bbox 裁剪后旋转为 upright，再提取 embedding；
ReID candidate crop：bbox 裁剪后旋转为 upright，再与 gallery 比对；
诊断 gallery：保存 upright crop，便于 PC 侧复盘；
debug/session_info：明确标记 reidCrop=upright。
```
...
安装新版 APK 后重新采集一组 `cartfollow_diagnostics`，流程至少包含：
...
```text
新 session 的 gallery 是否仍被标记为 landscape_or_rotated；
target_return 后 frames_to_reacquire / frames_to_follow 是否缩短；
belief_high_bbox_failed 是否减少；
非目标转绿是否仍出现；
若仍卡住，是 bbox gate、prediction、candidate switch，还是 hard STOP。
```
