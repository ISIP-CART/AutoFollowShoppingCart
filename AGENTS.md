...
这个策略属于“安全优先的有限重定位”，不是连续追踪搜索。这里需要区分 `motion_stop` 与 hard `STOP`：目标丢失或身份不确定时应先取消线速度、停止前进，但仍可在短时窗口内原地低速搜索和重捕获；hard `STOP` 应作为搜索失败、风险过高或安全异常后的兜底终态。
...
## 2026-09-05 用户授权更新：有限拐角探行

用户明确接受“目标被遮挡后，允许有条件的短距离前进”，本条更新此前目标丢失策略中禁止所有前进的表述：默认丢失仍先取消前进；仅 R3 三路已接入、安装朝向明确、已有原目标退出方向与货架拐角证据时，允许独立控制器低速分段沿边探行。2026-09-05 实测进一步确认，目标身份可靠且当前画面新鲜时，R3 状态 1/2 的无效回波按“未发现明显障碍”处理，不限速且不阻止视觉直行、差速或对准；已有有效近障碍锁存不能由无效回波解除。目标遮挡后的探行仍不得把无效回波当作净空。不得将陌生人物当作原目标。保留急停、通信与相机超时、明确传感器故障、总时限/转角限制，失败后停车。本次近距离跟随保留原初始化流程，以宽度启发式配合中央限速，不把脚部测距等同人物距离。候选参数待实车验证。
