...
- 当前上位机主线已经明确为：
  ...
  - Android TFLite ReID 已在 Human Cart Simulator 中跑通，下一步重点是 `TargetTrackManager + IdentityBeliefAccumulator`，防止单帧 ReID 高分导致目标切换，并提升目标返回后的安全重捕获能力。
...
当前仓库已经包含文档规划、ReID PC 研究工作区和 OpenBot Android 子仓库指针。`dev/OpenBot/android` 中已实现 Human Cart Simulator、PersonCropCollector、PersonSequenceCollector、阶段 A 行为层和阶段 B ReID 首版接入；硬件底盘真实控制仍未接通。
